...
-- this wild cards Only works for string!
...
-- the underscores represent the number of missing letters
...
-- From the employee table, choose those that do not fall into this .....
SELECT *
FROM humanresources.employee
...
SELECT
	...
	ROUND(AVG(vacationhours)) AS rounded_avg,
...
-- left join is the best for debugging
...
-- COALESCE is used to handle NULL value
...
FROM humanresources.employee AS employee --this is the primary table!!
LEFT JOIN person.person AS person
	ON employee.businessentityid = person.businessentityid
	-- the result of the top entire thing will be considered the left table
LEFT JOIN person.emailaddress AS emailaddress 
	ON employee.businessentityid = emailaddressid;
...
-- It depends on the relationship that both the table present for the left join share
-- If it is a one-to-one relatioinship, the chance of having duplicates is unlikely
-- However, if it is a one-to-many relationship, there could be a chance for duplicates to be present

-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

-- JOINS: RIGHT
-- Write a query to retrieve all sales orders and their corresponding customers. 
-- If a sales order exists without an associated customer, 
...
FULL OUTER JOIN sales.salesorderheader AS salesorderheader    --FULL OUTER JOIN is the same as FULL JOIN
...
FROM sales.salesorderheader
WHERE territoryid = 1
	AND EXTRACT(YEAR FROM orderdate) = 2011;  --ADDING IN THE CONDITION
...
SELECT 
	...
	CAST(person.businessentityid AS VARCHAR (100)) AS varchar_id, --VARCHAR is a string of character

	UPPER(person.lastname) AS upperlastname,
	LOWER(person.lastname) AS lowerlastname,
	LENGTH(person.lastname) AS firstnamelength,

	LEFT(emailaddress.emailaddress,10) AS startermail,
	RIGHT(emailaddress.emailaddress,10) AS endmail,
